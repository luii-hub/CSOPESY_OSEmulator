...
Process::Process() : id(0), totalInstructions(0), memorySize(0), pageSize(0), cyclesInCore(0) {
...
int Process::getCyclesInCore() const {
    return cyclesInCore;
}

void Process::incrementCyclesInCore() {
    cyclesInCore++;
}

void Process::resetCyclesInCore() {
    cyclesInCore = 0;
}
void Process::execute() {
    if (currentInstruction < totalInstructions) {
        currentInstruction++;
        incrementCyclesInCore();
    } else {
        finished = true;
    }
}