...
std::vector<MemoryBlock> PagingAllocator::getProccessMemoryBlocks(void) const {
	std::vector<MemoryBlock> result;

	for (size_t i = 0; i < memory.size(); i += 1) {
		if (memory[i] != -1) {
			result.emplace_back(i * pageSize, pageSize, memory[i]);
		}
	}
