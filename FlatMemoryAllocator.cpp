...
void FlatMemoryAllocator::initialize(ConfigurationManager* newConfigManager) {
    ...
    memoryBlocks.emplace_back(0, memorySize, -1); // Initialize with a single free block of the entire memory
}

void printBlockInfo(const MemoryBlock& block) {
	std::cout << "Start: " << block.start
		<< "\nSize: " << block.size
		<< "\nStatus: " << (block.pid == -1 ? "Free" : "Allocated") << std::endl << std::endl;
}
...
bool FlatMemoryAllocator::allocate(Process process) {
    ...
    for (auto& block : memoryBlocks) {
        ...
        if (block.pid == -1 && block.size >= processSize) {

            int currBlockSize = block.size;

            block.pid = process.getID();
...
void FlatMemoryAllocator::mergeFreeBlocks() {
    ...
    for (size_t i = 0; i < memoryBlocks.size() - 1; ++i) {
        if (memoryBlocks[i].pid == -1 && memoryBlocks[i + 1].pid == -1) { // Check if current block and next block are free
...
int FlatMemoryAllocator::getInactiveMemory(const std::unordered_set<int>& runningProcessIDs) {
    ...
    for (const auto& block : memoryBlocks) {
        if (block.pid != -1 && runningProcessIDs.find(block.start) == runningProcessIDs.end()) {
...
std::vector<MemoryBlock> FlatMemoryAllocator::getProccessMemoryBlocks() const {
	std::vector<MemoryBlock> used_blocks;
	for (const auto& block : memoryBlocks) {
		if (block.pid != -1) {
			used_blocks.emplace_back(block);
		}
	}
	return used_blocks;
}
