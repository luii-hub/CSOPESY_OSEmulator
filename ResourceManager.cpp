...
bool ResourceManager::initialize(ConfigurationManager* newConfigManager) {	
	...
	if (scheduler.initialize(configManager, this) && memoryManager.initialize(configManager, &scheduler)) {
		running = true;
...
void ResourceManager::generateMemoryStampFile(long long cycle) {
	char filename[512];
	snprintf(filename, sizeof(filename)-1, "memory_stamp_%lld.txt", cycle);
	...
	file << "Timestamp: (" << std::put_time(&localTime, "%m/%d/%Y %I:%M:%S %p") << ")\n";
	...
	std::vector<MemoryBlock> memoryBlocks = memoryManager.getProccessMemoryBlocks();
	std::sort(
		memoryBlocks.begin(), 
		memoryBlocks.end(),
	...
	long long unfragmented_memory = 0;
	for (int i = 0; i < (int)(memoryBlocks.size()-1); i += 1) {
		unfragmented_memory += memoryBlocks[i].start - (memoryBlocks[i+1].start + memoryBlocks[i+1].size);
	}
	unfragmented_memory += memoryBlocks[memoryBlocks.size()-1].start;
	file << "Total external fragmentation: " << unfragmented_memory << "\n\n";

	file << "----end---- = " << memoryBlocks[0].start + memoryBlocks[0].size << "\n\n";

	for (int i = 0; i < (int)(memoryBlocks.size()); i += 1) {
		file << memoryBlocks[i].start + memoryBlocks[i].size << '\n';
		file << 'P' << memoryBlocks[i].pid << '\n';
		file << memoryBlocks[i].start << '\n';
		file << '\n';
	}
