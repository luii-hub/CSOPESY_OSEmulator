        ...
        } else if (command.substr(0,6) == "screen") 
        {
            ...
            else if (command.substr(0,9) == "screen -r")
            {
                ...
                { 
                    ...
                    // Check if the process name exists, else check if the process is finished, else switch screen
                    if (consoleManager.getResourceManager().processExists(processName)) {
                        //if it exists, check if the process is finished

                        if (consoleManager.isProcessFinished(processName)) { // if the process is finished, there will be no screen
                            cout << "Process " << processName << " has already finished. " << endl;
                        }
                        else if (!consoleManager.isProcessFinished(processName)) { // if the process is not finished, switch screen
                        	consoleManager.switchScreen("PROCESS_SCREEN_" + processName); 
                        }

                    }
                    else { // if the process does not exist
                    	cout << "Process " << processName << " does not exist. " << endl;
                    }
                }
            }

            if (command == "screen") {
                cout << "\nscreen command usage: " << endl;
                cout << "screen -ls - List all screens" << endl;
                cout << "screen -s [processName] - Create a new process screen" << endl;
                cout << "screen -r [processName] - Switch to a process screen" << endl;
            }
