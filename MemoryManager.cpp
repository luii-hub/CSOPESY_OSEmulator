...
std::vector<MemoryBlock> MemoryManager::getProccessMemoryBlocks() const {
    if (allocationType == "flat") {
       return flatAllocator.getProccessMemoryBlocks();
    }
    else {
        return pagingAllocator.getProccessMemoryBlocks();
    }
}
